const PostBody = ({ caption, image, commentsLength }) => {
  return (
    <>
      <p className="text-2xl">{caption}</p>
      {image && <img src={image} className="w-full h-100 object-cover mt-3" />}
      <div className="w-full h-8 flex items-center px-3 my-3">
        ...
        <div className="w-full flex justify-between">
          ...
          <p className="ml-3 text-gray-500">{commentsLength} comment</p>
        </div>
      </div>
    </>
  );
